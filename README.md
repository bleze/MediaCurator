...
- **NZBGet integration** *(opt-in)* — Configure your NZBGet server in **Settings → Downloads** to see its live download queue right above the library, as a slim progress band and a full queue dialog (with the same 4K/3D/Edition badges the library cards use). Optionally, MediaCurator can automatically Quick Scan and Quick Analyze as soon as a download's post-processing (unpack, par-repair, sort/rename scripts) finishes, so newly downloaded movies show up ready for review without lifting a finger. Built on a provider-agnostic download-client interface, so support for other download clients can be added the same way.
