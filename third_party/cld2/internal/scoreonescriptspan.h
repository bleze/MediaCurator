...
// NOTE: struct is named (not anonymous) as a local MSVC-conformance patch —
// MSVC's strict frontend (C7626) rejects an anonymous class/struct that
// declares member functions inside a typedef; GCC/Clang accept it as an
// extension. Reapply this if re-vendoring from upstream.
typedef struct LangBoosts {
   int32 n;
...
typedef struct ScoringHitBuffer {
  ULScript ulscript;        // langprobs below are with respect to this script
