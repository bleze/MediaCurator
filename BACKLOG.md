...
- [ ] `[P2]` **Savings estimate split brain — `j.saved_bytes` vs live `estimateSavingBytes()`** — Two independent code paths compute the estimated savings for a proposed job and they can diverge badly, causing sorting, the bulk summary dialog, the footer totals, and calibration data to all disagree with what the badge on the card actually shows. Root causes: (1) `FileDecision::estimatedSavingBytes()` (called by `AnalyzeWorker` at job-creation time) iterates only `decision.tracks`, which the `RuleEngine` populates with audio+subtitle streams only — video is never evaluated, so its declared bitrate (e.g. 30–40 Mbps for a HEVC remux) never enters the denominator; (2) the `qMax` file-bitrate floor in `estimateSavingBytes()` was added later, so jobs created before the formula change carry wildly inflated values in `j.saved_bytes`; (3) `toggleStream()` does call `updateJobSavedBytes()` to patch the stored value, but only after a user manually toggles a track. Fix plan: (a) change `FileDecision::estimatedSavingBytes()` to accept the full stream list (including video) so it computes the same value as the badge from the moment of analysis; (b) add a one-time startup migration that recalculates `saved_bytes` for all `proposed` jobs using the current formula and the live stream data in the DB; (c) consider making `saved_bytes` a purely stored column for *done* jobs (actual measured bytes freed) and a computed/virtual value for *proposed* jobs so the two meanings can't be conflated. Until this is fully resolved, `McJobListModel` works around stale `j.saved_bytes` by sorting `LargestSavingsFirst` in-memory using the live formula.
