...
- [x] `[P2]` **Media categories (Movie / TV / Documentary / Misc)** — `files.media_type` column; TMDB enrichment classifies via `/find` (movie_results + tv_results) and genre 99 → Documentary; title search tries movie/TV with path heuristics; ImdbSearchDialog uses `/search/multi`; filter pills Movies/TV/Docs/Misc; card type chip; right-click Set Category override
...
- [ ] `[P2]` **Download scene NFOs from srrDB** — some local scene NFOs are wrong/stale (mismatched release content from a copy-pasted template) or internally corrupted beyond recovery (see Bug Tracker below); add a way to fetch a correct one instead of just disregarding it. srrDB (`api.srrdb.com/v1`) is a public JSON API for the scene-release archive: `GET /v1/search/r:<release-name>` (derived from the video filename minus extension) to confirm the release exists, then `GET /v1/nfo/<release-name>` for its NFO download link. Should be an explicit, user-triggered action per file (e.g. right-click → "Download Scene NFO…"), not automatic during scan — it's a community-run API, not an official service, so keep it conservatively rate-limited/cached rather than batched across the whole library. Filenames renamed by Radarr/Sonarr/Plex-naming won't match the original release name exactly, so a fuzzy/best-effort match (or a manual "pick from results" step) will be needed. No conflict with the existing Kodi NFO writer to design around — confirmed `writeMovieNfo` already never truncates a non-Kodi NFO (only patches the IMDb id in place), so downloading a scene NFO is just a normal (explicit, confirmed) overwrite of that one file, and the writer's existing patch-in-place logic keeps working on it afterward unchanged.
...
- [x] `[P2]` **Sidecar subtitle prefix matching attaches subs to the wrong movie** — Fixed 2026-07-18: an unrecognized non-empty token in the remainder disqualifies the candidate (Kodi convention); "default" accepted as a known flag; 3-letter language assumption now requires strictly alphabetic tokens. — `ScanWorker.cpp:269-305` accepts any candidate whose base name `startsWith(baseName)` and silently ignores unrecognized tokens: `Rocky II.en.srt` becomes an `eng` stream of `Rocky.mkv`. An unrecognized non-empty token should disqualify the candidate (Kodi convention).
- [x] `[P2]` **`readImdbId` falls back to ANY .nfo in the folder** — Fixed 2026-07-18: tries `movie.nfo` (Kodi folder convention) first; the any-.nfo sweep now only runs when the video is the folder's sole video file. — `NfoParser.cpp:64-68`; in flat library layouts movie A inherits movie B's IMDb id → wrong poster/title/NFO propagation. Restrict fallback to single-video folders (or `movie.nfo`); also: regex takes the first `tt\d{7,8}` anywhere in free-text scene NFOs.

**Crashes / races:**

- [x] `[P2]` **AnalyzeWorker use-after-free** — Fixed 2026-07-18: both Analyze paths now delete the worker from `QThread::finished` (SimulateWorker pattern); `onAnalyzeFinished` no longer nulls the pointer. — `McMainWindow.cpp:2861` (+ Quick Analyze at :2914) still uses self-`deleteLater` on `finished`; worker is destroyed on its own thread before the queued main-thread handler nulls the pointer, and Cancel dereferences the dangling pointer. The SimulateWorker fix (deleteLater on `QThread::finished`, :2983-2995) was never ported.
- [x] `[P2]` **`ImdbSearchDialog` abort() loops mutate hashes mid-iteration (UB)** — Fixed 2026-07-18: new `abortResultRequests()` helper detaches handlers, empties the members, then aborts local copies; both call sites use it. — `onSearch()` (:664-673) and `searchByExistingImdbId()` (:734-743) call `abort()` (synchronous `finished`) inside range-fors over `m_thumbReplyByRow`/`m_backdropReplyByRow`/`m_prefetchByRow`, whose handlers `remove()` from the same hash. Destructor/`fetchPosterImages` already use the safe `disconnect` pattern.
- [x] `[P2]` **Stale `m_acceptAfterFetch` accepts the wrong movie** — Fixed 2026-07-18: `onResultSelectionChanged` disarms the flag; batch auto-select unaffected (it arms after `setCurrentRow`). — armed by double-click/Enter while an ID prefetch is in flight (`ImdbSearchDialog.cpp:495,1032`), never cleared on selection change; if the user then selects row B, B's prefetch landing silently `accept()`s with B's metadata committed (NFO, poster, original language).
- [x] `[P2]` **Finalize pool task captures raw `this`** — Fixed 2026-07-18: `m_cancelRequested` is now a `shared_ptr<atomic<bool>>` held by the pool task; all `this`-captures replaced with `QPointer` (`self`), results marshaled via `QMetaObject::invokeMethod(qApp, ...)` with a null check on the main thread. Same pattern applied to `JobQueue::commitReview`'s pool task. — `RemuxJob.cpp:258` (and `JobQueue::commitReview` :1191); app shutdown during a NAS copy-back destroys the RemuxJob while the task dereferences `m_cancelRequested` / invokes methods on it.

**Lifecycle / consistency:**

- [x] `[P2]` **24 h update-check throttle is dead (int truncation)** — Fixed 2026-07-18: `0LL` default at the call site, and `jsonToVariant` now falls back to qint64 for out-of-int-range values instead of a UB narrowing cast. — default `0` at `UpdateChecker.cpp:70` is `int`, so `AppSettings::jsonToVariant` (:68) does `static_cast<int>(1.78e12)` (UB → INT_MIN); every launch hits the GitHub API. Fix: `0LL` default and/or range-check in `jsonToVariant`.
- [x] `[P2]` **Worker DB connection registry** — Fixed 2026-07-18: rekeyed on a `QThreadStorage<ThreadDbConnection*>` with a monotonic connection-name counter (never the OS thread id), so a recycled thread id can no longer inherit a dead thread's connection; `ThreadDbConnection`'s destructor closes and `removeDatabase()`s automatically when the owning thread exits; a failed `open()` now immediately unregisters the connection instead of leaving a poisoned entry. `m_connMutex` removed (no longer needed). — `DatabaseManager.cpp:96-120`: was keyed on recycled OS thread ids (a new thread can inherit a dead thread's connection — unsupported in Qt), never `removeDatabase()`d (leak per thread), and a failed `open()` left a poisoned never-opened registered connection that all future queries from that thread id used silently.
- [x] `[P2]` **Failed rename leaves a movie-sized .tmp forever** — Fixed 2026-07-18: the genuine-failure branch now removes the .tmp and says so in the job log. — `RemuxJob.cpp:397-410`: when `std::filesystem::rename` fails (source held open by Plex/Kodi), the job is marked failed but `outputPath` is never deleted or surfaced (exit≠0 branch does clean up). Delete it or route to needs-review like the mismatch path.
- [x] `[P2]` **`McJobListModel::applyFilter` diff bails on reorder, leaving a stale view** — Fixed 2026-07-18: the "unreachable" branch now falls back to a full `beginResetModel()` instead of `break`. — the "unreachable" `break` (`McJobListModel.cpp:656-680`) is reachable because `toggleStream()` changes savings without resorting while `reload()` re-sorts; next refresh leaves `m_entries` stale (old streams/status/check states). Fall back to `beginResetModel()` there.
- [x] `[P3]` **"Queue All" acts on filtered/paged entries but is enabled from DB-wide counts** — Fixed 2026-07-18: `onQueueAll()` now queries `DatabaseManager::jobIdsByStatus("proposed")` (new DB-wide method) and `proposedJobFileIds()` directly instead of the model's filtered/paged `m_entries`; `updateJob()` already pulls any out-of-page job into the model's master list on demand, so the in-memory view stays correct regardless of the active status filter or startup paging. — `McJobPanel.cpp:1603` iterated visible `m_entries`; with status filter "Done" it silently did nothing, and during startup paging (first 50 + live jobs only) it queued a fraction of the proposed jobs the footer advertised.
- [x] `[P3]` **`allFiles()`/`filesWithoutAnyJob()` hardcode `m_db`** — Fixed 2026-07-18: both use `connection()`. — `DatabaseManager.cpp:637,664`; latent thread bug — every other query goes through `connection()`. Cheap swap.
